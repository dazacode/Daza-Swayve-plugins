...
///   covering the whole operation including any follow-up requests, and a
///   breach surfaces as `SwayvePluginTimeoutException`;
  ...
  try {
    ...
    return await raced.timeout(
      ...
      onTimeout: () => throw SwayvePluginTimeoutException(
        'YouTube Music: $operation exceeded its '
    ...
    throw SwayvePluginTimeoutException(
      'YouTube Music: $operation timed out.',
      limit: timeout,
      cause: error,
    );
  } catch (error) {
    ...
    throw SwayvePluginUnavailableException(
      'YouTube Music: $operation failed unexpectedly.',
      cause: error,
    );
  }
}
...
Never throwForStatus(SwayveHttpResponse response, Uri url) {
  if (response.statusCode == 429) {
    throw SwayvePluginRateLimitedException(
      'YouTube Music rate limited a request to ${url.host}.',
